...
import com.asparck.eclipse.multicursor.MultiCursorPlugin;

public class MultiEditLinesHandler extends AbstractHandler {
	...
	/**
	 * This is mostly based off code from org.eclipse.jdt.internal.ui.text.correction.proposals.LinkedNamesAssistProposal
	 */
	private void goGoMultiCursor(ISourceViewer viewer) throws BadLocationException {
		...
		while (!currLine.equals(endLine) && currLine.getLength() + currLine.getOffset() + 1 < document.getLength()) {
			currLine = document.getLineInformationOfOffset(currLine.getLength() + currLine.getOffset() + 2); //FIXME this 2 is windows specific for \r\n chars
			lines.add(currLine);
		}
